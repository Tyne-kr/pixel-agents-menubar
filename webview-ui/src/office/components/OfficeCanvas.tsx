...
export function OfficeCanvas({
  ...
  const handleMouseDown = useCallback(
    (e: React.MouseEvent) => {
      unlockAudio();
      // Middle mouse button (button 1) OR left-click when not in edit mode → pan
      if (e.button === 1 || (e.button === 0 && !isEditMode)) {
        e.preventDefault();
  ...
  const handleMouseUp = useCallback(
    (e: React.MouseEvent) => {
      // Stop panning on any button release (middle OR left-click pan)
      if (isPanningRef.current) {
  ...
  // Wheel: zoom (Electron) or Ctrl+wheel zoom / plain wheel pan (VS Code)
  const isElectronApp = typeof window !== 'undefined' && 'pixelAgentsAPI' in window;
  const handleWheel = useCallback(
    (e: React.WheelEvent) => {
      e.preventDefault();
      if (isElectronApp) {
        // Electron: plain wheel = smooth zoom (0.1 step), no Ctrl needed
        const delta = e.deltaY > 0 ? -0.1 : 0.1;
        const newZoom = Math.max(ZOOM_MIN, Math.min(ZOOM_MAX, Math.round((zoom + delta) * 10) / 10));
        if (newZoom !== zoom) {
          onZoomChange(newZoom);
        }
      } else if (e.ctrlKey || e.metaKey) {
        // VS Code: Ctrl+wheel to zoom (integer steps)
    ...
    [zoom, onZoomChange, officeState, panRef, clampPan, isElectronApp],
  );
  ...
  // Fit to Screen: calculate zoom so the entire office fills the canvas
  useEffect(() => {
    const handler = (e: MessageEvent) => {
      if (e.data?.type !== 'fitToScreen') return;
      // Ensure canvas size matches current container (may have changed due to fullscreen)
      resizeCanvas();
      const container = containerRef.current;
      if (!container) return;
      const layout = officeState.getLayout();
      ...
      // The tile map includes walls above the floor. The "visible office" that
      // humans see is only the floor + wall decoration area.
      // Scan tiles to find the actual floor bounding box.
      const tiles = officeState.getLayout().tiles;
      const cols = officeState.getLayout().cols;
      const rows = officeState.getLayout().rows;
      let minR = rows, maxR = 0, minC = cols, maxC = 0;
      for (let r = 0; r < rows; r++) {
        for (let c = 0; c < cols; c++) {
          const t = tiles[r * cols + c];
          // Non-wall, non-void tiles (floor tiles are typically < 200)
          if (t !== undefined && t !== 255 && t !== 0) {
            if (r < minR) minR = r;
            if (r > maxR) maxR = r;
            if (c < minC) minC = c;
            if (c > maxC) maxC = c;
      ...
      // Include 2 rows above for wall decorations (shelves, art, plants)
      const visMinR = Math.max(0, minR - 2);
      const visMaxR = Math.min(rows - 1, maxR);
      const visMinC = Math.max(0, minC - 1);
      const visMaxC = Math.min(cols - 1, maxC + 1);

      const visW = (visMaxC - visMinC + 1) * TILE_SIZE;
      const visH = (visMaxR - visMinR + 1) * TILE_SIZE;

      // Zoom to fit the visible office area
      const fitZoomX = canvasW / visW;
      const fitZoomY = canvasH / visH;
      const fitZoom = Math.min(fitZoomX, fitZoomY);
      const clampedZoom = Math.max(ZOOM_MIN, Math.min(ZOOM_MAX, fitZoom));

      onZoomChange(clampedZoom);

      // Pan to center the visible office area (not the whole tile map)
      const visCenterX = ((visMinC + visMaxC + 1) / 2) * TILE_SIZE;
      const visCenterY = ((visMinR + visMaxR + 1) / 2) * TILE_SIZE;
      const mapCenterX = (cols * TILE_SIZE) / 2;
      const mapCenterY = (rows * TILE_SIZE) / 2;
      panRef.current = {
        x: (mapCenterX - visCenterX) * clampedZoom,
        y: (mapCenterY - visCenterY) * clampedZoom,
      };
      officeState.cameraFollowId = null;
    };
  ...
  // Global mouseup: stop panning even if mouse is released outside canvas
  useEffect(() => {
    const globalMouseUp = () => {
      if (isPanningRef.current) {
        isPanningRef.current = false;
